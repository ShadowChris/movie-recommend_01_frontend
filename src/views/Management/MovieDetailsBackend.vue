...
export default {
  ...
  created() {
    this.$http.get( this.global.baseURL + this.global.oldManURL.getOne + "/" + this.$route.params.id).then(result => {
      console.log(result)
  ...
  methods: {
    updateBlog() {
      this.$http
          .put(this.global.baseURL + this.global.oldManURL.update + "/" + this.id, {
            id: this.id,
